...
public class CalculationController {
    ...
    @GetMapping("/multiple")
    public ResponseEntity<?> multiple(@RequestParam String number1, @RequestParam String number2) {
        {
            try {
                int result = calculationService.multiple(number1, number2);
                return ResponseEntity.ok(result);
            } catch (Exception ex) {
                ex.printStackTrace();
                return ResponseEntity.badRequest().body(ex.getMessage());
    ...
    @GetMapping("/divide")
    public ResponseEntity<?> divide(@RequestParam String number1, @RequestParam String number2){
        {
            int divide = calculationService.minus(number1, number2);
            if (divide != -1) {
